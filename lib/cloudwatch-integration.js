var LIMITS = {
  MAX_EVENT_MSG_SIZE_BYTES: 250000, // The real max size is 262144, we leave some room for overhead on each message
  MAX_BATCH_SIZE_BYTES: 900000, // We leave some fudge factor here too.
};
    ...
    lib.getToken(
      ...
      function (err, token) {
        ...
        var payload = {
          ...
          logEvents: logEvents.splice(0, entryIndex).filter((ev) => !!ev),
        };
...
lib.getToken = function (
  ...
  function executeCalls(index, results, cb) {
    if (index >= calls.length) {
      var groupPresent = calls.length > 1 ? results[0] : true,
        stream = calls.length === 1 ? results[0] : results[1];
    ...
    calls[index](function (err, result) {
      if (err) {
        cb(err);
        return;
      }
      results.push(result);
      executeCalls(index + 1, results, cb);
    });
  }
